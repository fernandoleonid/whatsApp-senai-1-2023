...
const criaContato = (contato) => {
    const blockChat = document.createElement('button')
    blockChat.classList.add('blockChat')
    blockChat.setAttribute('type', 'button')
    blockChat.setAttribute('name', contato.name)
    ...
    blockChat.append(imgbx, detail)
    imgbx.append(image)
    detail.append(listHead, previewMessage)
    listHead.append(name, time)
    previewMessage.append(preview)

    blockChat.addEventListener("click", () => openConversation(blockChat.getAttribute('name')))
